...
| `paid_out` / `paid_out_today` | **Gross coin winnings credited**: the full payout, stake included, for player-vs-player wins and tournament prizes. **Games against a bot are excluded** from every money figure, winner and top-winner list. Confirmed rows only, deduplicated. **Not withdrawals.** Private and banned players are counted but never named. | `transactions` `match_win` |
