/**
 * The Terms of Service and Privacy Policy text, as data.
 *
 * Lifted out of the two pages that used to hold it inline, because the age and
 * terms modal has to show the same words. It cannot link to those pages — it
 * is a full-screen gate that blocks navigation, so the links opened nothing —
 * and it must not carry its own copy, because two copies of a legal document
 * is one that quietly stops matching what people actually agreed to.
 */
...
export const PRIVACY_SECTIONS = [
  ...
  },
  {
    ...
    body: 'We do not store the contents of in-game chat; messages are relayed between players in real time and are not written to our database. We do not collect payment card details, government identification, physical address, or phone number — deposits and withdrawals are made in cryptocurrency and we have no card rail. We do not use advertising trackers, and we do not sell or rent your personal information to anyone, for any purpose.',
  },
  {
  ...
  },
  {
    ...
    body: 'We share only what each provider needs to do its job: our authentication and database provider stores your account; our hosting providers run the site; cryptocurrency exchange partners convert funds during deposits and withdrawals; and public blockchain services are queried using your deposit addresses to detect incoming payments. We may also disclose information where we are legally required to, or where it is necessary to investigate fraud or protect the platform and its users.',
  },
  {
