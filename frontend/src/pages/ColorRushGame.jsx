...
export default function ColorRushGame() {
  ...
  const privateRematch = usePrivateRematch(socket, 'color_rush_match_found');
  ...
  // game then came up with the top of the course cut off. This locks both
  ...
  useEffect(() => {
    ...
    socket.on('color_rush_queue_joined', () => { setPhase('queue'); setStatusMsg('Waiting for opponent…'); });
    socket.on('color_rush_queue_left',   () => { setPhase('lobby'); setStatusMsg(''); });
    ...
    socket.on('color_rush_match_found', ({ roomId: rid, opponent: opp, entryFee: fee, currency }) => {
      roomIdRef.current = rid;
      setRoomId(rid);
      setOpponent(opp);
      setMyMs(0); setOppMs(0); setOppDied(false); setDied(false); setResult(null); diedRef.current = false;
      setPhase('queue');
      playMatchFound();
      if ((fee ?? 0) > 0) {
    ...
    // Private rooms and friend invites.
    ...
    socket.on('color_rush_countdown', ({ count }) => { setCountdown(count); playCountdown(); });

    socket.on('color_rush_start', ({ seed: s }) => {
      setSeed(s);
      setCountdown(0);
      setPhase('playing');
      playGo();
    ...
    socket.on('color_rush_opponent_progress', ({ ms }) => setOppMs(ms));
    socket.on('color_rush_opponent_died', ({ ms }) => { setOppMs(ms); setOppDied(true); });
    // The opponent died while ahead: a fixed window to beat their score.
    socket.on('color_rush_catchup', ({ seconds, targetScore }) => {
      setCatchup({ endsAt: Date.now() + seconds * 1000, targetScore });
    });
    socket.on('color_rush_died', ({ ms }) => { setMyMs(ms); setDied(true); });
    ...
    return () => {
      ...
      [
        'color_rush_queue_joined','color_rush_queue_left','match_cancelled','color_rush_match_found',
        'color_rush_countdown','color_rush_start','color_rush_opponent_progress','color_rush_opponent_died',
        'color_rush_died','color_rush_result','opponent_disconnected','error','color_rush_catchup',
  ...
  function joinQueueWith(feeArg, curArg) {
    ...
    setStatusMsg('');
    setPhase('queue');
    socket?.emit('join_color_rush_queue', { entryFee: fee, currency: cur });
  }
  function leaveQueue() { socket?.emit('leave_color_rush_queue'); setPhase('lobby'); setStatusMsg(''); }
  function playVsBot() {
    lastModeRef.current = 'bot_paid';
    lastSettingsRef.current = { entryFee, currency: betCurrency };
    socket?.emit('play_color_rush_vs_bot', { entryFee, currency: betCurrency });
  }
  function playVsBotFree() {
    lastModeRef.current = 'bot_free';
    lastSettingsRef.current = { entryFee: 0, currency: 'coins' };
    socket?.emit('play_color_rush_vs_bot', { entryFee: 0, currency: 'coins' });
  }
  function createPrivate(fee, cur) { socket?.emit('create_private_room', { gameType: 'colorRush', entryFee: fee, currency: cur }); }
  function joinPrivate(code)       { socket?.emit('join_private_room', { gameType: 'colorRush', code }); }

  // Gameplay callbacks from the canvas — the run reports score + time survived.
  const onProgress = (score, ms) => {
    setMyMs(ms);
    if (roomIdRef.current) socket?.emit('color_rush_progress', { roomId: roomIdRef.current, ms, score });
  };
  ...
  function clearMatch() {
    ...
    setMyMs(0); setOppMs(0); setDied(false); setOppDied(false); setStatusMsg('');
    setPrivateCode(''); setInvitedFriend(null); setCatchup(null); diedRef.current = false;
  }
  ...
  function playAgain() {
    ...
    if (mode === 'pvp') {
      ...
      socket.emit('play_color_rush_vs_bot', mode === 'bot_free'
  ...
  return (
    ...
    >
      <GameLobby
        ...
        statusMsg={statusMsg}
        gameType="colorRush"
        liveCount={playerCounts?.['color-rush'] ?? 0}
      />
    </div>
  );
}
