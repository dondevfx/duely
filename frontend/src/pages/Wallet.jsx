...
// ── Supported coins ───────────────────────────────────────────────────
const COINS = [
  { id: 'usdttrc20', label: 'USDT',  network: 'TRC-20',    best: true },
  { id: 'sol',       label: 'SOL',   network: 'Solana' },
  { id: 'btc',       label: 'BTC',   network: 'Bitcoin' },
  { id: 'xrp',       label: 'XRP',   network: 'XRPL',      needsTag: true, tagLabel: 'Destination Tag' },
  { id: 'eth',       label: 'ETH',   network: 'Ethereum' },
  { id: 'bnbbsc',    label: 'BNB',   network: 'BSC' },
  { id: 'ltc',       label: 'LTC',   network: 'Litecoin' },
  { id: 'doge',      label: 'DOGE',  network: 'Dogecoin' },
  { id: 'trx',       label: 'TRX',   network: 'TRON' },
  { id: 'matic',     label: 'MATIC', network: 'Polygon' },
  { id: 'ada',       label: 'ADA',   network: 'Cardano',   needsTag: true, tagLabel: 'Memo' },
  { id: 'avaxc',     label: 'AVAX',  network: 'Avalanche' },
];
...
export default function Wallet() {
  ...
  const [depPayment, setDepPayment] = useState(null);
  ...
  const [witExtraId, setWitExtraId]     = useState('');
  ...
  async function handleCreatePayment() {
    ...
    setDepPolling(false);
    try {
      const data = await api.post('/wallet/create-payment', { coin: depCoin.id });
      setDepPayment(data);
      startPolling();
    } catch (err) {
  ...
  function startPolling() {
    ...
    const interval = setInterval(async () => {
      attempts++;
      if (attempts > 60) { clearInterval(interval); setDepPolling(false); return; }
      try {
  ...
  async function handleWithdraw() {
    if (!witAmountUsd || parseFloat(witAmountUsd) < 5 || !witAddress.trim()) return;
    ...
    <div className="min-h-screen bg-bg pt-16" style={{ opacity: ready ? 1 : 0, transition: 'opacity 0.35s ease' }}>
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="bg-surface border border-surfaceLight rounded-2xl p-6 mb-6">
          ...
          <p className="text-sm text-muted mb-4">Select a coin, get a deposit address, and send any amount $5 and above.</p>
              ...
              <div className="bg-bg border border-primary/30 rounded-xl p-5 space-y-4 animate-slide-down">
                ...
                <div className="flex justify-center">
                  <div className="bg-white p-3 rounded-xl">
                    <img
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(depPayment.pay_address)}&margin=2`}
                      alt="Deposit QR code"
                      width={180}
                      height={180}
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button onClick={() => copyAddr(depPayment.pay_address)} className="text-xs text-primary hover:underline font-semibold">
                      {depCopied ? '✓ Copied!' : 'Copy'}
                    </button>
                  </div>
                  <button onClick={() => copyAddr(depPayment.pay_address)} className="w-full text-left">
                    <code className="block text-xs font-mono text-white break-all bg-surface px-3 py-3 rounded-lg hover:border-primary border border-transparent transition-colors">
                      {depPayment.pay_address}
                    </code>
                  </button>
                </div>

                {/* Destination tag / memo for XRP, ADA */}
                {depPayment.extra_id && (
                  <div className="bg-danger/10 border border-danger/30 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-danger font-bold uppercase">⚠️ {depCoin.tagLabel || 'Destination Tag'} Required</span>
                      <button onClick={() => copyText(depPayment.extra_id)} className="text-xs text-primary hover:underline">Copy</button>
                    </div>
                    <code className="block text-sm font-mono text-white bg-bg px-3 py-2 rounded-lg">{depPayment.extra_id}</code>
                    <p className="text-xs text-danger mt-1">Do not forget this — funds will be lost without it.</p>
                  </div>
                )}
                ...
                <button
                  onClick={() => { setDepPayment(null); setDepPolling(false); }}
        ...
        <div className="bg-surface border border-surfaceLight rounded-2xl p-6 mb-6">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <p className="text-xs text-muted mb-2">Select coin</p>
              <CoinGrid selected={witCoin} onSelect={c => { setWitCoin(c); setWitExtraId(''); }} />
            </div>
            ...
            {witCoin.needsTag && (
              <div>
                <label className="block text-xs text-muted mb-1">
                  {witCoin.tagLabel} <span className="text-danger">required</span>
                </label>
                <input
                  value={witExtraId}
                  onChange={e => setWitExtraId(e.target.value)}
                  placeholder={witCoin.tagLabel}
                  className="w-full bg-bg border border-surfaceLight rounded-lg px-4 py-3 text-white placeholder-muted text-sm font-mono focus:outline-none focus:border-primary transition-colors"
                />
              </div>
            )}
            ...
            <GlowButton
              ...
              disabled={
                witLoading ||
                !witAmountUsd ||
                parseFloat(witAmountUsd) < 5 ||
                !witAddress.trim() ||
                (witCoin.needsTag && !witExtraId.trim()) ||
