...
-- Returns { winnerPayout, prizePool, fee }
...
-- Accumulates platform fee share into fee_balance (separate from c_coins).
-- Admin collects via collect_admin_fees.
...
-- ── Rakeback ─────────────────────────────────────────────────────
-- 0.5% of prize pool per match split evenly: 1/3 instant, 1/3 daily, 1/3 weekly.
-- Each player gets 0.25% of prize pool (= 0.5% total across both players).
-- The backend calls add_rakeback_instant/daily/weekly each with (perPlayer / 3).

-- Convenience function — adds p_amount / 3 to each bucket atomically.
-- Still here for backward compatibility; new code calls the three below directly.
...
-- Individual bucket increments (called by rakebackService.js)
...
-- Instant claim — 5 minute cooldown
