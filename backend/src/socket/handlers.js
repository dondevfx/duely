...
const userQueues = new Set(); // userId → currently in a queue (prevents dual-tab double-join)
  ...
  // Is this account in a live match on a DIFFERENT connection, or still in a
  ...
  function _deliverInvite(inviteId) {
    ...
    for (const s of _socketsForUser(inv.toUserId)) {
      s.emit('game_invite', {
        ...
        fromAvatar: inv.fromAvatar ?? null, fromColor: inv.fromColor ?? null,
  ...
  gameEvents.on('friend_request', ({ toUserId, fromUserId, fromUsername, fromAvatar, fromColor }) => {
    for (const [, sock] of io.sockets.sockets) {
      if (sock._authenticatedUserId === toUserId) {
        sock.emit('friend_request', { fromUserId, fromUsername, fromAvatar, fromColor });
      }
    }
  });
  ...
  io.on('connection', (socket) => {
      ...
      socket.on = (event, handler) => {
        ...
        return _on(event, async (...args) => {
          if (authenticatedUser) {
            ...
            // One game per ACCOUNT, not per connection. _inLiveRoom checks the
    ...
    socket.on('join_block_blast_queue', async ({ entryFee = 0, currency = 'coins' }) => {
      ...
      const { data: profile } = await supabase
        .from('profiles').select('c_coins,diamonds,elo,username,avatar_url,profile_color').eq('id', authenticatedUser.userId).single();
      ...
      const player = { socketId: socket.id, userId: authenticatedUser.userId, username: profile.username, elo: profile.elo, avatarUrl: profile.avatar_url ?? null, profileColor: profile.profile_color ?? null, entryFee, currency, isDemo: authenticatedUser.isDemo || false };
    ...
    socket.on('play_block_blast_vs_bot', async ({ entryFee = 0, currency = 'coins' } = {}) => {
      ...
      try {
        ...
        const { data: profile } = await supabase.from('profiles').select('elo,username,c_coins,diamonds,avatar_url,profile_color').eq('id', authenticatedUser.userId).single();
        if (entryFee > 0) {
          try {
    ...
    socket.on('invite_friend', async ({ friendId, gameType, entryFee = 0, currency = 'coins' }) => {
      ...
      pendingInvites.set(inviteId, {
        ...
        fromAvatar: authenticatedUser.avatar_url ?? null,
        fromColor:  authenticatedUser.profile_color ?? null,
    ...
    socket.on('join_private_room', async ({ gameType, code }) => {
      ...
      if (entryFee > 0) {
        const { data: pf } = await supabase.from('profiles').select('c_coins,diamonds,elo,username,avatar_url,profile_color').eq('id', authenticatedUser.userId).single();
      ...
      const { data: profile2 } = await supabase.from('profiles').select('elo,username,avatar_url,profile_color').eq('id', authenticatedUser.userId).single();
    ...
    socket.on('join_coin_flip_queue', async ({ entryFee = 0, currency = 'coins', side }) => {
      ...
      if (inMatchOrQueue(authenticatedUser.userId))
        return socket.emit('error', { message: 'Already in a queue' });
      const { data: profile } = await supabase.from('profiles').select('c_coins,diamonds,elo,username,avatar_url,profile_color').eq('id', authenticatedUser.userId).single();
      ...
      const player = { socketId: socket.id, userId: authenticatedUser.userId, username: profile.username, elo: profile.elo, avatarUrl: profile.avatar_url ?? null, profileColor: profile.profile_color ?? null, entryFee, currency, side, isDemo: authenticatedUser.isDemo || false };
      userQueues.add(authenticatedUser.userId);
